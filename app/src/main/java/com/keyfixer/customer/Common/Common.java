...
public class Common {
        ...
        token_tbl.orderByKey().equalTo(fixerid).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot postSnapshot:dataSnapshot.getChildren()){
                    final Token token1 = postSnapshot.getValue(Token.class); // get token object from database with key
                    //make raw payload - convert latlng to json
                    final String customer_token = FirebaseInstanceId.getInstance().getToken();
    ...
    public static void sendCancelToFixer(String fixerid , final IFCMService ifcmService , final Context context) {
